...
tmux bind-key m set-option -g mouse

# Pass mouse events through to root table so mouse works without Shift
tmux \
	bind-key -T "$keytable" MouseDown1Pane   "select-pane -t = ; send-keys -M" \; \
	bind-key -T "$keytable" MouseDrag1Pane   "select-pane -t = ; if-shell -F '#{mouse_any_flag}' 'send-keys -M' 'copy-mode -M'" \; \
	bind-key -T "$keytable" WheelUpPane      "select-pane -t = ; if-shell -F '#{mouse_any_flag}' 'send-keys -M' 'if-shell -F \"#{alternate_on}\" \"send-keys Up Up Up\" \"copy-mode -e ; send-keys -M\"'" \; \
	bind-key -T "$keytable" WheelDownPane    "select-pane -t = ; if-shell -F '#{mouse_any_flag}' 'send-keys -M' 'if-shell -F \"#{alternate_on}\" \"send-keys Down Down Down\" \"send-keys -M\"'" \; \
	bind-key -T "$keytable" MouseDown1Status "select-window -t =" \; \
	bind-key -T "$keytable" MouseDrag1Border "resize-pane -M"

# If you have sessions, and put #S in the status bar, click or
# scroll on the session name to cycle through sessions
tmux \
	bind-key -T "$keytable" MouseUp1StatusLeft   switch-client -n \; \
	bind-key -T "$keytable" WheelDownStatusLeft   switch-client -n \; \
	bind-key -T "$keytable" WheelUpStatusLeft     switch-client -p
...
tmux bind-key -T "$keytable" M-S "split-window -vf -c '#{pane_current_path}'"
tmux bind-key -T "$keytable" M-s "split-window -v  -c '#{pane_current_path}'"
